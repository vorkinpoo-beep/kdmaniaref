...
def start_command(message):
    ...
    if db.is_contest_ended():
        ...
        if first_50_winner:
            text += f"⚡ <b>ПЕРВЫЙ, КТО НАБРАЛ 50 РЕФЕРАЛОВ:</b>\n"
            text += f"@{first_50_winner.get('username', 'N/A')} - {first_50_winner['referrals_count']} рефералов\n"
            text += f"Приз: {PRIZE_FIRST_50}\n\n"
                ...
                if is_valid:
                    ...
                    if db.add_referral(referrer_id, user_id):
                        ...
                        if referrals_count >= 50:
                            first_50_winner = db.get_first_50_winner()
                            if first_50_winner and first_50_winner['user_id'] == referrer_id:
                                # Уведомление о победе за 50 рефералов (только если он первый)
                                try:
                                    winner_text = f"🎉 ПОЗДРАВЛЯЕМ!\n\n"
                                    winner_text += f"Вы первым достигли 50 рефералов!\n\n"
                                    winner_text += f"🏆 Ваш приз: {PRIZE_FIRST_50}"
    ...
    text += f"⚡ <b>Первый, кто наберет 50 рефералов</b>: NFT Instant Ramen\n{PRIZE_FIRST_50}\n\n"
    text += "📋 <b>ПРАВИЛА:</b>\n"
    ...
    text += "• Приз за 50 рефералов получает тот, кто первым достигнет этой отметки\n"
